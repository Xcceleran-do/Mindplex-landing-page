...
<section class="roadmap my-32 mx-6">
	<div class="relative w-full">
		...
		<div class="absolute -top-10 flex flex-row justify-evenly w-full md:-top-14">
			{#each roadmap as { slug, background_color, color }}
				<div>
					<div>
					...
					<a href={`/roadmap/${slug}`} rel="external">
						...
						></div>
						<div
							class={`w-8 h-8 absolute top-7 ml-3 rounded-full md:top-12 ${slug == active ? 'opacity-100' : 'opacity-20'}`}
							style="background-color: {background_color};"
						></div>
					</a>
				</div>
	...
	{#each filteredContent as { step, content_title, section_color,desc,color }}
		<div class=" w-full flex flex-col justify-center items-center">
			<div>
				<h1 class="text-4xl mt-20 text-center" style="color: {section_color};">
					{step}
				</h1>
			</div>
